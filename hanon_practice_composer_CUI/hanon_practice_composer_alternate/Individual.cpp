...
void Individual::print(std::string fileName) {
	...
	octave = "o5";
	//octave = "o4";
	//if (data->selectedNum == 2 || data->selectedNum == 0 || data->selectedNum == 1 || data->selectedNum == 3)
		//octave = "o5";
	fout << "l8"<< octave << "v150\n";
	...
	for (int i = 0; i < data->chromLen; i++) {
		...
		if (r <= mutateProb) {
			...
			if (tmp2 == "Y" || tmp2.substr(0, 1) == "X") {//yxの処理
				...
				if (data->noteNum[1]*7 <=i && i <=data->noteNum[1]*7+data->noteNum[1]-1 && tmp2.substr(0, 1) == data->convNotes[0]//最後の小節にある主音だったら(前半)
					||data->noteNum[1]*15 <= i && i <=data->noteNum[1]*15+data->noteNum[1]-1 && tmp2.substr(0,1) == data->convNotes[0]//最後の小節にある主音だったら(後半)
					|| data->selectedNum == 4 && chrom[i - 1] == "Y") {//半音階で一個前の値がYだったら
					r = 0;
					continue;
				}
...
void Individual::loadChrom(int Num) {
	std::ifstream fin;
	std::string tmp;
	int seekcomma = 0;
	int lastcomma = -1;
	...
	for (int i = 0; i < data->chromLen; i++) {
		do {
			seekcomma++;
			counter++;
		} while (tmp.substr(seekcomma, 1) != ",");
		chrom[i] = tmp.substr(lastcomma+1, counter);
		counter = -1;
		lastcomma = seekcomma;
	}
	printChrom();
